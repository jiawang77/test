...
def dataset_builder(args, config):
    from datasets.PCNDataset import TreeDataset
    if hasattr(config, '_base_') and isinstance(config._base_, dict):
        for key in ['DATA_ROOT', 'N_POINTS', 'N_COMPLETE_POINTS', 'SKELETON_POINTS']:
            if not hasattr(config, key) and hasattr(config._base_, key):
                setattr(config, key, getattr(config._base_, key))
    for key, default in [
        ('DATA_ROOT', 'PCN'),
        ('N_POINTS', 2048),
        ('N_COMPLETE_POINTS', 16384),
        ('SKELETON_POINTS', 512)
    ]:
        if not hasattr(config, key):
            setattr(config, key, default)
    dataset = TreeDataset(config, subset=config.others.subset)
...
def model_builder(config):
    ...
    if model_type == 'TreePoinTr':
        from models.PoinTr import TreePoinTr
        model = TreePoinTr(config)  # 传递整个 config 对象
    elif model_type == 'PoinTr':
        from models.PoinTr import PoinTr
        model = PoinTr(config)
    else:
        raise ValueError(f"Unknown model type: {model_type}")
    print_log(f'[ModelBuilder] Model: {model_type}', logger=logger)
    return model
